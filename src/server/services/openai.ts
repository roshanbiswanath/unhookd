...
export class OpenAIService {
  ...
  async planActivity(draft: OnboardingDraft, recentActivities: Array<{ title: string; helpfulness: string }>, pullBefore: number | null): Promise<ActivityContract> {
    ...
    const response = await this.client.responses.create({
      model: this.model,
      input: [{ role: "user", content: [{ type: "input_text", text: `Create one safe, 30-180 second activity from this approved Unhookd profile. It must be observable by a phone camera or timer and must not require equipment. The current pull intensity is ${intensity} (${pullBefore ?? "not rated"}/5). For low pull, offer a light grounding action; for moderate pull, choose a short interruption; for high pull, choose the most engaging safe approved alternative. Select from approvedActivities and avoid repeating a recent title when another approved alternative is available. When cameraEnabled is true, strongly prefer a count-based activity with clear repeated movement (for example, wall push-ups, squats, or overhead reaches) over a static hold, because it gives Live Coach something observable to report. Use a duration activity only when a countable alternative is unsuitable. Profile: ${JSON.stringify(draft)}. Recent activities and outcomes: ${JSON.stringify(recentActivities)}` }] }],
